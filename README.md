...
非常欢迎社区伙伴参与 QuickPick 的建设！无论是提交 Bug 反馈、讨论新功能建议，还是直接贡献代码 Pull Request (PR)，都备受期待。

1. **阅读贡献规范**：详细规范请参阅 [CONTRIBUTING.md](CONTRIBUTING.md)；
2. **核心安全红线**：所有代码改动必须严格遵循**源目录只读**原则，绝不产生修改或覆盖原片行为；
3. **PR 提交前门禁**：在发起 PR 之前，请务必在本地运行并通过全套自动化质量门禁：
   ```bash
   npm run gate
   ```
   该命令会自动验证前端 Vitest 测试、生产打包、Rust Clippy 与单元测试、以及基线完整性。
